...
import ship.f.engine.shared.utils.serverdrivenui2.config.state.models.Id2.ZoneId2
...
import ship.f.engine.shared.utils.serverdrivenui2.state.ShimmerState2
import ship.f.engine.shared.utils.serverdrivenui2.state.SpacerState2
...
abstract class Zone2<D : DomainEvent6>(val update: KClass<D>) {
    ...
    lateinit var sendComputed: (ComputedSduiDomain<*>, String, Int) -> Unit
    lateinit var sendFailed: (FailedSduiDomain<*>, String, Int) -> Unit
    var fallbackUI: State2 = ShimmerState2(size = Size2.Fixed2(0, 0))
    var failureUI: State2 = SpacerState2(size = Size2.Fixed2(0, 0))
    ...
    suspend fun request3(input: ViewRequest6): List<RefState2> {
        ...
        val refs = input.id.scopes.mapNotNull { scope ->
...
//                sduiLog("UninitiatedViewRequest6 $name for $input", tag = "ZoneGraph2 > request3")
                ...
                if (createOrJoinRequest(input = initiatedInput, publish = sendComputed)) return refs
                else initiatedInput
            }
...
//                sduiLog("DomainViewRequest6 $name for $initialisedInput", tag = "ZoneGraph2 > request3")
                ...
                addOutput(requestId = initialisedInput.requestId, domainSdui = updatedSduiDomain, publish = sendComputed)
            }

            is FailedDomainViewRequest6 -> {
                // TODO What does this mean for cached results
                // TODO we should probably add this to the input and not create a send2 method
                sduiLog("FailedDomainViewRequest6 $name for $initialisedInput", tag = "ZoneGraph2 > request3")
                val sduiDomain = sduiDomainMutex.withLock {
                    sduiDomains[initialisedInput.focusDomainId] as? FailedSduiDomain<D>
                        ?: error("sduiDomain ${initialisedInput.focusDomainId} should exist for $initialisedInput")
                }
                addError(requestId = initialisedInput.requestId, domainSdui = sduiDomain, publish = sendFailed)
            }
...
//        sduiLog("Augmenting request for $input with ${domain::class}", tag = "ZoneGraph2 > augmentRequest")
    ...
    suspend fun failedDomainRequest(input: DomainViewRequest6, domainId: String) {
        val pendingDomain = sduiDomains[domainId] as? PendingSduiDomain<D> ?: return
        val sdui = SDUIInput2(
            id = Id2.MetaId2(name = input.id.name, scope = input.focusDomainId),
            states = listOf(
                failureUI.c(
                    id = Id2.StateId2(
                        name = input.id.name,
                        scope = input.focusDomainId,
                        isGlobal = true
                    )
                )
            )
        )
        sduiDomains[domainId] = FailedSduiDomain(domainId = pendingDomain.domainId, requesters = pendingDomain.requesters, sdui = sdui)
        pendingDomain.requesters.forEach {
            request3(
                FailedDomainViewRequest6(
                    id = input.id,
                    ctx = input.ctx,
                    listCtx = input.listCtx,
                    requesterId = it,
                    requestId = input.requestId,
                    focusDomainId = input.focusDomainId
                )
            )
        }
    }
    ...
    companion object {
        ...
        suspend fun createOrJoinRequest(
            input: InitiatedViewRequest6,
        ...
        suspend fun addError(
            requestId: String,
            domainSdui: FailedSduiDomain<*>,
            publish: suspend (FailedSduiDomain<*>, String, Int) -> Unit
        ) {
            requestBuilderMutex.withLock {
                val requestBuilder = requestBuilders[requestId]
                    ?: sduiLog("request builder for $requestId has gone missing, potentially due to invalidation").let { return }
                requestBuilders[requestId] = requestBuilder.copy(errors = (requestBuilder.errors + domainSdui).distinct())
                requestBuilder.requesters.forEach { publish(domainSdui, it, requestBuilder.version) }
            }
        }

        private suspend fun addRequester(
            requestId: String,
            requesterId: String,
            publish: suspend (ComputedSduiDomain<*>, String, Int) -> Unit
        ) {
        ...
        data class RequestBuilder(
            ...
            val errors: List<FailedSduiDomain<*>>,
            val version: Int = 1,
        )
    }
}